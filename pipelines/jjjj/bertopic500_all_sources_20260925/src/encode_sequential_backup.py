...
start = time.time(); completed = []; audit_rows = []; totals = {'documents': 0, 'chunks': 0, 'content_tokens': 0, 'unusable_documents': 0}
for part in iter_parts():
    out = OUT / (part.stem + '.npy')
    meta_path = OUT / (part.stem + '.json')
    if out.exists() and meta_path.exists():
        meta = json.loads(meta_path.read_text())
        assert meta['corpus_sha256'] == sha(part)
    ...
    meta = {'documents': len(rows), 'chunks': len(seq), 'content_tokens': int(sum(token_counts)), 'unusable_documents': int(sum(not r['usable'] for r in rows)), 'seconds': time.time()-t, 'corpus_sha256': sha(part), 'embedding_sha256': sha(out), 'created_utc': now()}
    dump(meta_path, meta)
    for k in totals: totals[k] += meta[k]
    completed.append(part.stem)
    dump(BASE / 'PROGRESS.json', {'stage': 'encoding', 'last_part': part.stem, **totals, 'seconds_current_run': time.time()-start, 'updated_utc': now()})
    print(part.stem, meta['documents'], 'chunks', meta['chunks'], 'sec', round(meta['seconds'], 1), 'total_docs', totals['documents'], flush=True)
